...
export class AddPersoneelComponent implements OnInit {
  ...
  constructor(private _personeelDataService: PersoneelDataService, private fb: FormBuilder, private router: Router,   private notificationService: NotificationServiceService,
...
(err: any) => console.log(err)
    ...
    */
    const personeel = new Personeel(this.personeelFG.value.name, this.personeelFG.value.name,
    ...
    this._personeelDataService.addNewPersoneel(personeel) .pipe(
      catchError((err) => {
        this.errorMessage = err;
        return EMPTY;
      })
    ).subscribe((persoon: Personeel) => {
      this.notificationService.success(`::  ${persoon.name} was succesfully added`); 
    });
; 
    this.router.navigate(['/personeel/list']);
    return false;
